...
# ─────────────────────────────────────────────
#  ТЕХНІЧНІ ФУНКЦІЇ (НАДІЙНИЙ С КРІНШОТ НА WINDOWS)
# ─────────────────────────────────────────────
...
def take_screenshot_cv():
    ...
    try:
        # Запитуємо стиснений PNG (ключ -p), це унеможливлює пошкодження байтів системними потоками Windows
        ...
        # Декодуємо отриманий масив байтів зображення в BGR формат OpenCV
...
def find_ore_template(scene_img, template_img, threshold=0.70):
    ...
    # Якщо збіг вище порогу, повертаємо координату центру
...
def main():
    ...
    while True:
        ...
        # Постійний моніторинг кнопки ОК для безпеки
        ...
        if state == STATE_SEARCHING:
            ...
            if ore_x is not None:
                ...
                if search_rotation_count < 12:
                    ...
                    print("[SEARCH] Повне коло пройдено. Робимо кроки наосліп вперед.")
            ...
            print("[WALK] Курс вірний. Крок вперед...")
            make_micro_step()
            time.sleep(0.1)
