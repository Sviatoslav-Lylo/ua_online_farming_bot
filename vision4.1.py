...
# ─────────────────────────────────────────────
#  UTILITY FUNCTIONS (RELIABLE SCREENSHOT ON WINDOWS)
# ─────────────────────────────────────────────
...
def take_screenshot_cv():
    ...
    try:
        # Request compressed PNG (-p) to avoid byte corruption by Windows streams
        ...
        # Decode the received byte array into an OpenCV BGR image
...
def find_ore_template(scene_img, template_img, threshold=0.70):
    ...
    # If the match is above threshold, return the center coordinate
...
def main():
    ...
    while True:
        ...
        # Constant monitoring of the OK button for safety
        ...
        if state == STATE_SEARCHING:
            ...
            if ore_x is not None:
                ...
                if search_rotation_count < 12:
                    ...
                    print("[SEARCH] Full circle scanned. Taking blind steps forward.")
            ...
            ore_x = find_ore_template(img, template_img)
            if ore_x is None:
                print("[AIM] Target lost while centering. Returning to scan.")
                state = STATE_SEARCHING
                continue
            ...
            if ore_x is None:
                print("[WALK] Text disappeared. Maybe we're close. Taking the final step...")
                make_micro_step()
                time.sleep(0.3)
